...
| **主要な充足点** | `physical-ai-core` への editable 依存採用、ADR-0001 制御アーキテクチャ確定、詳細な設計文書・ナレッジグラフ、GMP/Part 11/ALCOA+ 対応設計、Manstein ODE シミュレータ、`cell_culture` ドメインプラグイン、L1 決定的レシピ/ルールエンジン、ALCOA-lite 監査ログ・承認フロー、MQTT gateway / 仮想バイオリアクター |
| **主要な不足点** | L2 ベイズ最適化エンジン、多変数適応 MPC、Raman 閉ループ、凝集体画像 DL 品質代理指標、GMP IQ/OQ/PQ・電子署名・完全職員独立性 |
...
| 軸 | スコア | コメント |
|---|---|---|
| 情報源の充実度 | 4/5 | センサー設計・チャネル定義・at-line 計測計画が実装に落ちている。凝集体画像パイプラインは未実装 |
| 意思決定ループ完成度 | 4/5 | L1 決定的閉ループ（Sense→Decide→Act）が plant_model + レシピ/ルールエンジン + 承認フローで成立。L2/L3 は未実装 |
...
- L2 ベイズ最適化エンジン（Ax/BoTorch）を導入し、run 間の設定点最適化を開始する
- Nova FLEX2 / LADS / SiLA2 / OPC-UA gateway アダプタを実装し、at-line データを `event_store` に取り込む
- 凝集体画像解析パイプライン（Cellpose ベース）を追加し、径分布・形態メトリクスを L1/L2 入力にする
