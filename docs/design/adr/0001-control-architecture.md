...
- ~~目的関数の具体定義（qccrit→スカラ/多目的）~~ → **解決済**（kg_to_auto_cell.md §4.3, 2026-06-15）:
  per-run 連続スカラ `J = 収量(VCD/fold) × 生存率 × 多能性%陽性`（＋検証後 label-free 代理）、同一性/核型/残存未分化は
  offline/periodic な **safe/constrained BO 制約**。先例 Kanda 2022（単一目的 batch BO, GPyOpt, 88%改善）で iPSC 実証。
  残: 拡大用 label-free 非破壊代理の検証、weight 調整。
